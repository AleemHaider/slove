...
async findAllUpComming(user: UserEntity, id: number , page: string, limit: string) {
 ...
 select e.id as event_id ,e.artist_id ,e.venue_id,e.location,f.id as feedback_id, bc.id as booking_contract_id,bc.music_genre as booking_contract_music_genre,bc.start_time as booking_contract_start_time,
  ...
  ruc on ruq.country_id=ruc.id left join cities ruc2 on ruq.city_id=ruc2.id left join event e on bc.id = e.booking_contract_id left join feedback f on e.id = f.event_id where bc.start_time > NOW() and 
   ((u.id=${id} and b.booking_status='ACCEPTED' and bc.contract_status='ACCEPTED') OR (ru.id=${id} and b.booking_status='ACCEPTED' and bc.contract_status='ACCEPTED')) order by bc.start_time desc  ${pagination}
`);
  ...
  on uq.country_id = c.id left join cities c2 on uq.city_id = c2.id left join "user" ru on b.requested_user_id=ru.id where bc.start_time > NOW() and 
   (u.id=${id} and b.booking_status='ACCEPTED' and bc.contract_status='ACCEPTED') `,
    );
